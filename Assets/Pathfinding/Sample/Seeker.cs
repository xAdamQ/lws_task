...
public class Seeker : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (knots.Count != 0 && knotIndex < knots.Count)
        {
            ...
            {
                RaycastHit hit;
                bool isHitting = Physics.Raycast(transform.position, (knots[knotIndex].position - transform.position).normalized, out hit,
                    Vector3.Distance(transform.position, knots[knotIndex].position));
                if (isHitting && hit.collider.gameObject.tag != pathfinder.obstacleTag)
                {
                    ...
                    else if (!obstacle.isObstacle(pathfinder))
